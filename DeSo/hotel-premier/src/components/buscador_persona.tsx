...
export default function BuscadorPersona({ titulo, onBuscar, onSeleccionar, excluidos = [] }: Props) {
    ...
    const isExcluido = (p: ResultadoBusq) => {
        return excluidos.some(e => e.tipoDoc === p.tipoDoc && e.nroDoc === p.nroDoc);
    };

    return (
        <div className="border border-gray-200 p-5 rounded-xl bg-white dark:bg-gray-900 shadow-sm mb-6">
            <h3 className="text-lg font-bold mb-4 text-gray-800 dark:text-gray-100 border-b pb-2">{titulo}</h3>

            <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1fr_1fr_0.8fr_1fr] gap-3 mb-4">
                <input
                ...
                {/* CORRECCIÓN VISUAL: Select con fondo oscuro en dark mode */}
                <select
                    name="tipoDoc" value={form.tipoDoc} onChange={handleChange}
                    className="border border-gray-300 dark:border-gray-700 p-2 rounded-lg w-full text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                >
                ...
                <div className="flex gap-2">
                    <input
                        name="nroDoc" placeholder="Nro Doc" value={form.nroDoc} onChange={handleChange}
                    ...
                    {resultados.length === 0 ? (
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            {resultados.map((r, i) => {
                                ...
                                return (
                                    <tr key={i} className={`transition-colors ${disabled ? 'opacity-50 bg-gray-50 dark:bg-gray-800' : 'hover:bg-blue-50 dark:hover:bg-blue-900/20'}`}>
                                        <td className="px-3 py-2 font-medium text-gray-900 dark:text-white">{r.apellido}, {r.nombre}</td>
                                        <td className="px-3 py-2 text-gray-500 dark:text-gray-400">{r.tipoDoc} {r.nroDoc}</td>
                                        <td className="px-3 py-2 text-right">
                                            <button
                                                ...
                                                onClick={() => {
                                                    onSeleccionar(r);
                                                    setResultados([]);
                                                    setForm({ apellido: "", nombre: "", tipoDoc: "", nroDoc: "" });
                                                    setBusquedaRealizada(false);
                                                }}
                                                className={`text-xs font-bold px-3 py-1 rounded ${disabled ? "text-gray-400 cursor-not-allowed" : "text-white bg-green-600 hover:bg-green-700"}`}
                                            >
