...
export default function CancelarReserva() {
  ...
  const handleCancelarMasivo = async () => {
    ...
    setResultados((prev) =>
      prev.filter((r) => !seleccionadas.has(r.idReserva))
    );
  ...
  return (
    <div className="dark:bg-gray-950 dark:text-white p-5 min-h-screen">
      ...
      <div className="border border-gray-200 p-8 rounded-2xl bg-[#f5f7fa] dark:bg-gray-950 shadow-sm mb-6">
        <form
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex flex-col">
              ...
              <input
                ...
                className={`p-2.5 border border-[#ddd] rounded-xl dark:bg-gray-950 dark:text-white focus:outline-none focus:border-[#9ca9be] focus:ring-2 focus:ring-[#4a6491]/20 ${
                  error && formData.apellido === "" ? "border-red-500" : ""
                }`}
              />
            </div>
            ...
            <div className="flex items-end">
              <button
                type="submit"
                disabled={loading}
                className=" bg-green-400/70 text-white p-2 rounded-xl hover:bg-green-500 transition-colors disabled:opacity-50 flex items-center justify-center"
              >
                {loading ? (
                  "..."
                ) : (
